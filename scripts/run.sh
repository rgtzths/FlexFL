...
function run_command {
    ...
    sshpass -p "$PASSWORD" ssh $ARGS "$USERNAME@$IP" "screen -dmS fl $COMMAND"
}

echo "Running command on master..."
COMMAND="cd flexfl && source venv/bin/activate && flexfl --ip $MASTER_IP $RUN_ARGS"
sshpass -p "$PASSWORD" ssh $ARGS "$USERNAME@$MASTER_IP" "screen -dmS fl \"$COMMAND"
